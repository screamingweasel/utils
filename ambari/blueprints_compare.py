#!/usr/bin/python
###################################################################################################
# Description: Compare two blueprint files and optionally produce a json updates file for
#              consumption into merge_properties.py
...
def main():
	...
	blueprintFile2=args.blueprintFile2
	updateFile=args.updateFile

	msg = "Comparing %s (A) to %s (B)\n" % (blueprintFile1, blueprintFile2)
	sys.stderr.write(msg)

	blueprint1 = read_properties(blueprintFile1)
	configs1 = parse_configs(blueprint1["configurations"])

	blueprint2 = read_properties(blueprintFile2)
	...
	if (args.updateFile != None):
		...
		sys.stderr.write("Updates written to %s\n" % (updateFile))
	
	sys.stderr.write(str(len(diffs)) + " differences found\n")
